...
NMEAParser < 2 > parser;                            //Parser class with 2 handlers: Error Handler and GPGGA Handler
volatile bool NMEA_ERROR_FLAG = false;    //Parse Error Flag
char SMS_STRING[130];                              // String Buffer in which e load the GPS coordinates after parsing

void errorHandler() {
  //  uart_print("*** Error : \n\r");
  NMEA_ERROR_FLAG = true;                
  //uart_print(parser.error());
}

void GPGGAHandler() {
  NMEA_ERROR_FLAG = false;
 ...
 // gpgga GPGGA;
  ...
  //Unecessary information for our use
  /* 
